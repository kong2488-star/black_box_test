...
<!-- 프롬프트를 바꾸려면 coarse_probe.py / fine_probe.py 의 상수를 고치고 -->
<!-- `python modules/search/dump_prompts.py` 로 다시 찍는다. -->

# 탐침 프롬프트 (Coarse 1 + Fine 4)

**원본은 코드다.** 아래는 그 사본이며 커밋 전 재생성으로 일치를 유지한다.

| 프롬프트 | 원본 상수 | 해시(prompt_hash) |
|---|---|---|
| Coarse | `coarse_probe.py:PROMPT` (+ EVENT_DEFINITIONS·힌트 치환) | `1a77e2b1c5f3` |
| Fine 공통+델타 | `fine_probe.py:COMMON_BLOCK` + `DELTAS` | `cb96f8174ff3` (fine-p2) |

> 이 표의 해시가 jsonl(`probe_runs.jsonl`/`fine_probe_runs.jsonl`)의 `prompt_hash` 와
> 다르면 문서가 낡은 것이다 — 재생성하라.

---

## 1. Coarse — 후보 구간 탐색

- 검색 대상: `SIGNAL, CENTER_LINE_CROSSING, LANE_CHANGE, MOTORCYCLE_HELMET_NON_USE`
- 역할: 싸게 넓게 훑어 Fine 이 검증할 후보 span 을 뱉는다. Recall 우선.
- 아래는 `build_prompt()` 가 조립한 **실제 전송 원문**이다(힌트는 비어 있어 `(없음)`).
...
## 2. Fine — 정밀 검증 (공통 블록 + 델타 4)

Fine 프롬프트 = **공통 블록 1개 + 유형별 델타 4개**. `build_prompt(event_type, ...)` 가
델타의 4슬롯(정의/필수요건/성립축/primitive예시)을 공통 블록의 `{EVENT_DEFINITION}` ·
`{REQUIRED_CONDITIONS}` · `{AXIS}` 자리에 끼워 넣는다. 아래 공통 블록의 `{...}` 자리가
각 유형의 델타로 채워진다.
...
**primitive 이름 예시:** `RED_SIGNAL`, `AMBER_SIGNAL`, `GREEN_SIGNAL`, `STOP_LINE`, `TARGET_VEHICLE`, `RED_AT_CROSSING`, `AMBER_AT_CROSSING`, `GREEN_AT_CROSSING`

**`temporal_facts[].fact` 이름 예시:** `SIGNAL_TURNED_AMBER`, `SIGNAL_TURNED_RED`, `VEHICLE_CROSSED_STOP_LINE`, `CROSSING_AFTER_RED_ONSET`, `CROSSING_BEFORE_RED_ONSET`
...
**primitive 이름 예시:** `YELLOW_CENTER_LINE`, `YELLOW_DOUBLE_LINE`, `TARGET_VEHICLE`, `VEHICLE_BODY_OVER_LINE`

**`temporal_facts[].fact` 이름 예시:** `VEHICLE_TOUCHED_CENTER_LINE`, `VEHICLE_BODY_CROSSED_CENTER_LINE`, `VEHICLE_RETURNED_TO_OWN_LANE`

### 2.3 `SOLID_LINE_LANE_CHANGE`

> (Coarse 의 옛 이름 `LANE_CHANGE` 에서 매핑)
...
**primitive 이름 예시:** `WHITE_SOLID_LINE`, `WHITE_DASHED_LINE`, `TARGET_VEHICLE`, `VEHICLE_CROSSES_LINE`

**`temporal_facts[].fact` 이름 예시:** `VEHICLE_STARTED_LATERAL_MOVE`, `VEHICLE_CROSSED_LINE`, `VEHICLE_SETTLED_IN_ADJACENT_LANE`
...
**primitive 이름 예시:** `MOTORCYCLE_RIDER`, `HELMET_ON_RIDER`
